...
export const EmailPreview = ({ show, events, onCancel, onSend, recipientEmail = 'paramgogia25@outlook.com' }) => {
  if (!show) return null;
  
  // Inside the EmailPreview component
const sendEmail = async () => {
    try {
      // Format events for email with HTML for better formatting
      const eventsList = events.map((event) => {
        const date = event.date.toLocaleDateString();
        const time = event.date.toLocaleTimeString();
        const priorityClass = event.priority === 'high' ? 'event-item high-priority' : 'event-item';
        const priorityText = event.priority === 'high' ? ' <strong>(High Priority)</strong>' : '';
        
        return `<div class="${priorityClass}">
                  <strong>${event.title}</strong> - ${date} at ${time}${priorityText}
                </div>`;
      }).join('');
      ...
      const templateParams = {
        ...
        from_name: 'AI Scheduler',
        events_list: eventsList,
        message: `Here is your schedule as requested. You have ${events.length} upcoming event(s).`
      };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full">
        ...
        <div className="border rounded-md p-4 mb-4 bg-gray-50">
          <p className="mb-2"><strong>To:</strong> {recipientEmail}</p>
          <p className="mb-2"><strong>Subject:</strong> Your Calendar Schedule</p>
          <div className="border-t my-2"></div>
          <p className="mb-2">Here's your schedule:</p>
          <ul className="list-disc pl-4 space-y-1">
            {events.map((event, index) => (
              <li key={index}>
                <strong>{event.title}</strong> - {event.date.toLocaleDateString()} at {event.date.toLocaleTimeString()} 
                {event.priority === 'high' && <span className="ml-2 text-red-600 font-bold">(High Priority)</span>}
              </li>
            ))}
          </ul>
        </div>
        <div className="flex gap-2 justify-end">
          <button
            onClick={onCancel}
            className="bg-gray-300 text-gray-800 py-2 px-4 rounded hover:bg-gray-400 flex items-center"
          >
            Cancel
          </button>
          <button
